...
- `collect/sampling.py` (new, shared): `sample_by_year_and_author()` — equal quota per
  calendar year with redistribution of a thin year's unused quota, deterministic
  hash-of-id ordering within a year, a 2%-of-target author cap. Written as a standalone
  module (not duplicated) because `task.txt` says "same sampling rule as
  `20-corpus/01-git-history-collector`" and 01 doesn't exist yet — when it's built (on
  HOST), it should import this instead of re-implementing it.
- `collect/github_prs.py`: `fetch_pull_requests()` (pages `GET /repos/{repo}/pulls`
  ...
  only on a short page); `is_valid_pull_request()` / `parse_pull_request()` /
  `filter_pull_requests()` (merged, pre-2021, human, non-bot, non-AI-co-authored, non-empty
  body; reuses `collect.filters.{CUTOFF,has_ai_co_author}`).
...
- Tests: `tests/test_sampling.py`, `tests/test_pr_collector.py`, `tests/test_collect_prs.py`
  — unmerged/bot/AI-co-author/empty-body rejected, mid-page cutoff stop, pagination,
  rate-limit sleep+retry, 5xx retry+backoff, checkpoint resume without duplicating,
  manifest fields, `load_candidates` deduplicates by `id` (see below).

## Two real bugs found by actually running this against GitHub, not by review
...
`OSError`). Added `except (urllib.error.URLError, http.client.HTTPException,
TimeoutError)` alongside the existing 5xx retry, same backoff. Tests added
(`test_fetch_pull_requests_retries_after_a_truncated_read_then_succeeds`,
`..._raises_after_repeated_truncated_reads`), seen red before the fix.
...
request ever finishing). The plan log's own fix for that was "a hard wall-clock cutoff via
`ThreadPoolExecutor.result(timeout=...)`, independent of whatever urllib/socket-level
timeout misbehaved" — applied the same pattern here: `_fetch_page()` runs in a
`ThreadPoolExecutor`, `.result(timeout=request_timeout)` (default 60s) bounds it from the
caller's side regardless of what the socket is doing; `executor.shutdown(wait=False)` on
generator close so an abandoned slow thread never blocks the pipeline. Tests added
(`test_fetch_pull_requests_gives_up_waiting_on_a_dribbling_response_and_retries`,
`..._raises_after_repeated_stalls`), seen red before the fix (`TypeError: unexpected
keyword argument 'request_timeout'` for the first; a real hang for the underlying
mechanism, confirmed by running the actual collector against GitHub, not simulated).

**3. (found while designing, fixed before it could bite) `load_candidates` did not
deduplicate.** If a run is killed between writing a page's filtered records and saving
that page's checkpoint, a resume re-fetches and re-appends the same page. Rather than try
to make the write+checkpoint atomic (impossible to fully guarantee against an external
kill), `load_candidates()` now dedupes by `id`, keeping the first occurrence — the pipeline
is idempotent regardless of where an interruption lands. Test:
`test_load_candidates_deduplicates_by_id_keeping_the_first_occurrence`, seen red first.
...
`scripts/batch_rewrite.py`'s untested `main()`): all exit 0. 167 tests, coverage 100.00%
...
Launched detached (`nohup ... &`, `disown`) on MAC:
...
`kubernetes/kubernetes`; observed pace ~19 pages/minute (~1400 candidates/minute) once the
timeout fix was in place. `python/cpython` was smoke-tested earlier in isolation and has a
stale partial checkpoint (`data/raw_v2/prs/.candidates/cpython.jsonl`, 3464 candidates,
page 50) that the full run will resume correctly when it reaches cpython (10th in
`PR_REPOS`) — `load_candidates`'s new dedup makes this safe even though that partial file
predates fix 2.

**To check progress:** `tail -f data/raw_v2/prs/MANIFEST.json` for repos already finished
(written incrementally after each repo — see fix 3's sibling design decision in
`scripts/collect_prs.py`'s `run()`), or `wc -l data/raw_v2/prs/.candidates/*.jsonl` for the
repo currently in flight. `ps aux | grep collect_prs` to confirm it's still alive.
...
`labels.txt` set to `status:in_progress`: code complete, tested, gate green, two real bugs
found and fixed with evidence from actually running it, reverse control done — but
